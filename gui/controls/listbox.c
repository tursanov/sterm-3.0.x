...
static void listbox_draw_row(listbox_t *listbox, list_item_t *i, int left, int top, int w, int h) {
	SetBrushColor(screen, listbox->selected_item == i ? 
			(listbox->control.focused ? clNavy : clGray) : 
			(listbox->control.focused ? clRopnetBrown : clSilver));
	SetTextColor(screen, listbox->selected_item == i ? clWhite : clBlack);
...
static bool listbox_redraw_row(listbox_t *listbox, int index) {
	if (index >= listbox->top_index && index < listbox->top_index + listbox->max_items) {
		list_item_t *li = listbox->top_item;
		...
		listbox_draw_row(listbox, li, x, y, w, h);
...
bool listbox_get_data(listbox_t *listbox, int what, data_t *data) {
	...
	case 0:
		...
		return true;
	case 1:
		if (listbox->selected_item) {
			data->data = (void *)listbox->selected_item->obj;
			data->size = sizeof(listbox->selected_item);
		} else {
			data->data = NULL;
			data->size = sizeof(listbox->selected_item);
		}
