	...
	while (true) {
		if (cheque_execute()) {
			...
			if (sumN > 0) {
				...
				BEGIN_FORM(form, "Расчет сдачи:")
					FORM_ITEM_EDIT_TEXT(1031, "Принято наличными:", "0", FORM_INPUT_TYPE_MONEY, 16)
				...
				while (1) {
					...
					if (n < sumN) {
						...
						sprintf(buffer, "ПРИНЯТО: %.1lld.%.2lld\n"
							"%s: %.1lld.%.2lld\n"
							"СДАЧА: %.1lld.%.2lld\n",
							n / 100, n % 100,
							sumN > 0 ? "СУММА К ПРИЕМУ" : "СУММА К ВДАЧЕ",
							sn / 100, sn % 100,
							sumC / 100, sumC % 100);
			...
			while (_ad->clist.head) {
				...
				if (_ad->clist.count == 1) {
					...
					if (sumN != 0) {
						n += sprintf(p + n, "ОПЛАТА НАЛИЧНМИ\r\n");
						if (sumN > 0)
							n += sprintf(p + n, "ВСЕГО К ПРИЕМУ: \x5%.1lld.%.2lld РУБ.\r\n",
								sumN / 100, sumN % 100);
						else
							n += sprintf(p + n, "ВСЕГО К ВПЛАТЕ: \x5%.1lld.%.2lld РУБ.\r\n",
								sumN / 100, sumN % 100);
						if (sumI > 0) {
					...
					if (sumE != 0) {
						n += sprintf(p + n, "ОПЛАТА БЕЗНАЛИЧНМИ\r\n");
						if (sumE > 0)
							n += sprintf(p + n, "ВСЕГО К ПРИЕМУ: \x5%.1lld.%.2lld РУБ.\r\n",
								sumE / 100, sumE % 100);
						else
