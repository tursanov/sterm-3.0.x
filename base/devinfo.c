/* Определение устройств, подключенных к терминалу. (c) gsr 2018 */
...
static bool send_poll(int dev, uint32_t *timeout)
{
	...
	if (rc == -1)
		...
		fprintf(stderr, "%s: в %s записано %d байт вместо %d.\n", __func__,
...
static bool read_status(int dev, uint8_t *status, uint32_t *timeout)
{
	...
	*status = 0xff;
	...
	if (rc == -1)
		...
		fprintf(stderr, "%s: из %s прочитано %d байт вместо 3.\n",
...
static bool read_data(int dev, uint32_t *timeout)
{
	...
	if (rc == -1){
		...
		fprintf(stderr, "%s: из %s прочитано %d байт вместо 1.\n",
			__func__, dev_name, rc);
		return false;
	}
