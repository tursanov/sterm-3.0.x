/* Основные функции терминала. (c) gsr 2000-2025 */
...
/* Установка параметров iplir */
static bool set_iplir_params(void)
{
	bool ret = false;
	struct iplir_section *iplir_cfg;
	if (!has_iplir)
		return false;
	iplir_cfg = read_iplir_cfg(IPLIR_MAIN_CONF);
	if (iplir_cfg != NULL){
		if (add_iplir_ppp0(iplir_cfg)){
			if (can_set_active_server(iplir_cfg))
				set_active_server(iplir_cfg);
			ret = write_iplir_cfg(IPLIR_MAIN_CONF, iplir_cfg);
		}
	}
	return ret;
}
...
static void init_term(bool need_init)
{
	...
	}else if (has_iplir && !is_iplir_loaded()){
		system("insmod /lib/modules/" IPLIR_MOD1_NAME ".ko");
		system("insmod /lib/modules/" IPLIR_MOD_NAME ".ko");
		if (!is_iplir_loaded()){
			fprintf(stderr, "Ошибка загрузки модулей VipNet.\n");
			stop_iplir();
			has_iplir = false;
		}
	}
...
/* Обмен значений основного и альтернативного ip хост-ЭВМ */
static void do_swap_ips(void)
{
	char msg[128];
	struct iplir_section *iplir_cfg;
	bool flag;
	if (!has_iplir || !cfg.use_iplir){
		set_term_astate(ast_illegal);
		err_beep();
		return;
	}
	iplir_cfg = read_iplir_cfg(IPLIR_MAIN_CONF);
	flag = (iplir_cfg != NULL) && can_set_active_server(iplir_cfg);
	snprintf(msg, sizeof(msg), "Будет установлен новый адрес хост-ЭВМ\n"
		"системы \"Экспресс\"\n%s",
		inet_ntoa(dw2ip(cfg.use_p_ip ? cfg.x3_s_ip : cfg.x3_p_ip)));
	online = false;
	guess_term_state();
	push_term_info();
	hide_cursor();
	...
	set_term_busy(true);
	ClearScreen(clBlack);
	if (message_box("Установка нового адреса хост-ЭВМ", msg,
			dlg_yes_no, 1, al_center) == DLG_BTN_YES){
		ClearScreen(clBlack);
		if (message_box("Установка нового адреса хост-ЭВМ",
				"Вы действительно хотите установить новый адрес\n"
				"хост-ЭВМ \"Экспресс\"?", dlg_yes_no, 1,
				al_center) == DLG_BTN_YES){
			cfg.use_p_ip = !cfg.use_p_ip;
			if (flag){		/* можно изменить iplir.conf */
				stop_iplir();
/*
 * После остановки VipNet iplir.conf может измениться, поэтому необходимо
 * проверить его еще раз.
 */
				iplir_cfg = read_iplir_cfg(IPLIR_MAIN_CONF);
				if ((iplir_cfg != NULL) && can_set_active_server(iplir_cfg)){
					flag = set_active_server(iplir_cfg) &&
						write_iplir_cfg(IPLIR_MAIN_CONF, iplir_cfg);
					if (!flag)
						cfg.use_p_ip = !cfg.use_p_ip;
				}
				start_iplir();
			}else			/* не требуется менять iplir.conf */
				flag = true;
			if (flag){		/* произошло переключение между хостами */
				if (cfg.use_p_ip)
					xlog_write_ipchange(hxlog, cfg.x3_s_ip, cfg.x3_p_ip);
				else
					xlog_write_ipchange(hxlog, cfg.x3_p_ip, cfg.x3_s_ip);
				write_cfg();
			}else{
				ClearScreen(clBlack);
				message_box("Ошибка",
					"При попытке установить новый адрес хост-ЭВМ \"Экспресс\"\n"
						"произошла ошибка. Новый адрес не установлен.",
					dlg_yes, 0, al_center);
			}
		}
	}
