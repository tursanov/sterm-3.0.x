/* Проверка кода удаления лицензии ИПТ. (c) gsr 2006 */
...
static FILE *open_data(char *name)
{
	FILE *f = fopen(name, "r");
	if (f == NULL)
...
static bool check_number(char *number)
{
...
static void translate_code(char *code, uint8_t *buf)
{
	int i, j, c, v;
	for (i = j = 0; i < 32; i++){
		c = toupper(code[i]);
...
static bool check_code(char *number, char *code, time_t *t)
{
	uint8_t buf[16];
	uint8_t tmp[32];
	struct md5_hash md5;
...
static void print_rmlic(char *number, time_t t, bool ok)
{
	struct tm *tm = localtime(&t);
	if (ok)
		printf("\t%.*s: %.2d.%.2d.%.4d %.2d:%.2d:%.2d\n",
			TERM_NUMBER_LEN, number,
			tm->tm_mday, tm->tm_mon + 1, tm->tm_year + 1900,
			tm->tm_hour, tm->tm_min, tm->tm_sec);
	else
		printf("\t%.*s: неверный код\n", TERM_NUMBER_LEN, number);
}
