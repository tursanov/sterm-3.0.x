		...
		const char *text, uint8_t attr)
{
	...
	if (!validate_pos_text(text)){
		pos_set_error(POS_ERROR_CLASS_SCREEN, POS_ERR_DEPICT, (int)text);
		free(pos_text);
		screen->outside = true;
		return false;
	}
		...
		int width, int height)
{
	...
	if (!name || !items || !count)
		...
		pos_set_error(POS_ERROR_CLASS_SCREEN, POS_ERR_DEPICT, (int)name);
		screen->outside = true;
		return false;
	}	
	for (i = 0; i < count; i++){
...
static bool pos_parse_cur(struct pos_data_buf *buf, bool check_only)
{
	...
	if (!pos_read_word(buf, &x) || !pos_read_word(buf, &y)){
		...
		pos_set_error(POS_ERROR_CLASS_SCREEN, POS_ERR_DEPICT, (int)"");
		return false;
	}
...
static bool pos_parse_print(struct pos_data_buf *buf, bool check_only)
{
	...
	if (check_only || (n == 0)){
		if (validate_pos_text(str))
			...
			pos_set_error(POS_ERROR_CLASS_SCREEN, POS_ERR_DEPICT, (int)str);
			return false;
		}
	}else
