...
{
	return llog_write_rec(hlog, &code, 1, LLRT_ERROR, n_para) != -1UL;
}
...
bool llog_write_rd_error(struct log_handle *hlog, uint8_t code)
{
	...
	return llog_write_rec(hlog, &code, 1, LLRT_RD_ERROR, 0) != -1UL;
}
		...
		uint32_t n_para)
{
	...
	return llog_write_rec(hlog, data, sizeof(data),
		LLRT_PR_ERROR_BCODE, n_para) != -1UL;
}
...
{
	return llog_write_rec(hlog, &code, 1, LLRT_PR_ERROR, n_para) != -1UL;
}
...
bool llog_write_foreign(struct log_handle *hlog, uint8_t gaddr, uint8_t iaddr)
{
	...
	return llog_write_rec(hlog, (uint8_t *)&addr, sizeof(addr),
		LLRT_FOREIGN, 0) != -1UL;
}
...
{
	return llog_write_rec(hlog, NULL, 0, LLRT_ERASE_SD, 0) != -1UL;
}
...
{
	return llog_write_rec(hlog, &code, 1, LLRT_SD_ERROR, n_para) != -1UL;
}
...
bool llog_mark_rec_printed(struct log_handle *hlog, uint32_t number, uint32_t n_para)
{
	...
	if ((index != -1UL) && llog_read_rec(hlog, index)){
